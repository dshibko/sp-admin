...
class SettingsTermsForm extends TermsForm {
    ...
    public function initForm(User $user)
    {
        $term1 = $user->getTerm1();
        $term2 = $user->getTerm2();
        if ($this->has(ContentManager::TERMS_FIELDSET_NAME)){
            if (!is_null($term1) && $this->get(ContentManager::TERMS_FIELDSET_NAME)->has('term1')){
            ...
            if (!is_null($term2) && $this->get(ContentManager::TERMS_FIELDSET_NAME)->has('term2')){
